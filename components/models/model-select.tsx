...
export function ModelSelect({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies(isOpen): suppress
  useEffect(() => {
    setSearch("");
  }, [isOpen]);
...
function ModelList({
  ...
  useLayoutEffect(() => {
    ...
    const providerIndex = filtered.findIndex(
      (p) => p.id === selectedModel.provider.id
    );
    if (providerIndex < 0) {
      return;
    }
    ...
    for (let i = 0; i < providerIndex + 1; ++i) {
      ...
      index +=
        (i < providerIndex
          ...
          : filtered[i]?.models.findIndex((m) => m.id === selectedModel.id)) ??
        0;
    }
  ...
  return (
    <Virtualizer ref={ref}>
      {providerModels.flatMap((provider, index) => [
        ...(index > 0
          ? [<Separator className="my-2" key={`${provider.id}-sep`} />]
          : []),
        <CommandItem
...
function ModelCommandItem({
  ...
  const content = (
    <div className="space-y-4">
      ...
      {modelType === "language" && (
        <LanguageModelItemView model={model as LanguageModelInfo} />
      )}
      {modelType === "image" && (
        <ImageModelItemView model={model as ImageModelInfo} />
      )}
      {modelType === "speech" && (
        <SpeechModelItemView model={model as SpeechModelInfo} />
      )}
  ...
  if (isMobile) {
    return (
      <Popover>
        <Trigger isSelected={isSelected} model={model} onSelect={onSelect}>
          <PopoverTrigger asChild>
            <Button
              className="size-4"
              onClick={(e) => {
